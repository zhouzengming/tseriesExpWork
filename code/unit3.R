...
# 习题3-17
x<-read.table("../data/习题3.17数据.txt")
...
overshort<-ts(overshort$overshort,start=overshort$day[1])
plot(overshort)
#
...
# 例题3-11
# 读入数据，绘制时序图
b<-read.table("../data/file10.csv",sep=",",header = T)
...
# 例3-10续(2)
overshort<-read.table("../data/file9.csv",sep=",",header = T)
overshort<-ts(overshort)
overshort.fit<-arima(overshort,order = c(0,0,1))
...
# 例3-15
# 读入时序，绘制时序图
x<-read.table(file = "../data/file11.csv",sep=",",header = T)
x<-ts(x$yield,start = x$time[1])
plot(x)
# 序列白噪声检验
